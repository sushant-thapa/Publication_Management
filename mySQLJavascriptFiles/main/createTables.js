...
var commandCreateAuthorTable="CREATE TABLE author(author_email VARCHAR(50) UNIQUE NOT NULL,author_id INT AUTO_INCREMENT,first_name TEXT,last_name TEXT,PRIMARY KEY(author_id))"; 
// publication acts as the base class from which the others are derived

var commandCreatePublicationTable= "CREATE TABLE publication (publication_id INT AUTO_INCREMENT PRIMARY KEY,user_email VARCHAR(50), title VARCHAR(200), year INT, publisher TEXT,impact_factor FLOAT,city TEXT,doi TEXT,ranked BOOLEAN,rank TEXT,type_of_publication TEXT,FOREIGN KEY(user_email) REFERENCES user(user_email) ON DELETE CASCADE,UNIQUE(user_email,title))";
