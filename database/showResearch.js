...
function showResearchList(type_of_article, f3) {
  connection.query(
    `SELECT * FROM publication WHERE type_of_article="${type_of_article}"`,
    function(err, result, field) {
      ...
      for (let i = 0; i < arrayToBeSentToWebsite.length; i++) {
        connection.query(
          `SELECT author_email FROM contribution WHERE publication_id="${
            arrayToBeSentToWebsite[i].publication_id
          }"`,
          function(err, result, field) {
            if (err) throw err;
            ...
            if (i == arrayToBeSentToWebsite.length - 1) {
              for (
                let j = 0;
                j < arrayOfAuthorEmails.length;
                j++ // again iterating over the array of array of emails
              ) {
                for (let k = 0; k < arrayOfAuthorEmails[j].length; k++) {
                  connection.query(
                    `SELECT first_name,last_name FROM author WHERE author_email="${
                      arrayOfAuthorEmails[j][k].author_email
                    }"`,
                    function(err, result) {
                      if (err) throw err;
                      var tempObject = {
                      ...
                      if (
                        j == arrayOfAuthorEmails.length - 1 &&
                        k == arrayOfAuthorEmails[j].length - 1
                      ) {
                        ...
                        f3(arrayToBeSentToWebsite, arrayOfAuthorEmails);
                      }
                    }
                  );
                }
