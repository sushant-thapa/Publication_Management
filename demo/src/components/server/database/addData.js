...
function addPublication(arrayOfInputObjects) {
  // SQL commands
  var commandToAddAuthor =
    "INSERT INTO author (first_name,last_name,author_email) VALUES ?";
  ...
  var addDataInContribution =
    "INSERT INTO contribution (publication_id,author_email) values ?"; // mysql code to add in contribution table
  ...
  connection.query(insertToPublication, [values], function(err, result) {
    ...
    connection.query("select max(publication_id) from publication", function(
      err,
      result,
      fields
    ) {
      ...
      var arrayToBeAdded = [];
      connection.query("select author_email from author", function(
        err,
        result
      ) {
        if (err) throw err;
        ...
        for (let i = 0; i < arrayFromWebsite.length; i++) {
          ...
          if (result.length == 0) {
            ...
            for (
              let j = 0;
              j < result.length;
              j++ //iterates over the array from the database
            ) {
              if (arrayFromWebsite[i].author_email == result[j].author_email) {
                console.log(
                  "the array from website is " +
                    arrayFromWebsite[i].author_email
                );
                console.log(
                  "the checking author in database is " + result[j].author_email
                );
                counter++;
              }
              // note
            }
            if (counter == 0) arrayToBeAdded.push(arrayFromWebsite[i]);
          }
        }
        ...
        if (arrayToBeAdded.length != 0) {
          ...
          console.log(arrayToBeAdded);
          connection.query(commandToAddAuthor, [newArray], function(
            err,
            result
          ) {
            if (err) throw err;
            ...
            for (k = 0; k < arrayFromWebsite.length; k++) {
              valueInContribution[k] = [
                publication_id_latest,
                arrayFromWebsite[k].author_email
              ];
            }
            connection.query(
              addDataInContribution,
              [valueInContribution],
              function(err, result) {
                if (err) throw err;
