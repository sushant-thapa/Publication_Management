...
function deletePublication(publicationTitle) {
  ...
  //   doi: "this_is_doi",
  //   authors: [
  //     { first_name: "Ross", last_name: "Geller", email: "rossGeller@yahoo.com" },
  ...
  connection.query(
    `SELECT publication_id FROM publication where title="${publicationTitle}"`,
    function(err, result, fields) {
      ...
      connection.query(
        `DELETE FROM contribution where publication_id=${publicationIdToBeDeleted}`,
        function(err, result) {
          console.log("the entry has been deleted from the contribution table");
        }
      );
      connection.query(
        `DELETE FROM publication where publication_id=${publicationIdToBeDeleted}`,
        function(err, result) {
